...
import {signup, signUpWithGoogle, signUpWithApple} from "@/app/signup/actions";
...
function translateErrorMessage(error: string): string {
  ...
  const errorTranslations: Record<string, string> = {
    ...
    "Invalid email": "Невалидна е-пошта",
...
export function SignupForm({className, ...props}: React.ComponentProps<"div">) {
  ...
  async function onSubmit(data: SignupForm) {
    ...
    if (result?.error) {
      ...
      toast.error("Грешка при регистрација", {
        description: macedonianError,
      });
    } else {
      toast.success("Успешна регистрација!", {
        description: "Проверете ја вашата е-пошта за да ја потврдите вашата сметка",
      });
    }
  }
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <FormField
                    ...
                    render={({field}) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} type="email" placeholder="m@example.com" autoComplete="email" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({field}) => (
                      <FormItem>
                        <FormLabel>Потврди лозинка</FormLabel>
                        <FormControl>
                          <PasswordInput placeholder="••••••••" {...field} autoComplete="new-password" />
                        </FormControl>
                        <FormMessage />
              ...
              <div className="grid grid-cols-3 gap-4">
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (result?.error) {
                      const macedonianError = translateErrorMessage(result.error);
                      toast.error("Грешка при регистрација со Apple", {
                        description: macedonianError,
                      });
                    }
                  }}>
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (result?.error) {
                      const macedonianError = translateErrorMessage(result.error);
                      toast.error("Грешка при регистрација со Google", {
                        description: macedonianError,
                      });
                    }
                  }}>
                ...
                <Button
                  ...
                  className="w-full"
                  onClick={() => {
                    toast.error("Регистрација со Meta моментално не е достапна", {
                      description: "Обидете се со друг метод за регистрација",
                    });
                  }}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path
