...
async function optimizeHeroImage() {
  ...
  try {
    ...
    await sharp(inputPath).resize(1920, null, {fit: "inside", withoutEnlargement: true}).jpeg({quality: 80, progressive: true}).toFile(outputPath);

    // Create WebP version
    await sharp(inputPath).resize(1920, null, {fit: "inside", withoutEnlargement: true}).webp({quality: 80}).toFile(outputWebpPath);
